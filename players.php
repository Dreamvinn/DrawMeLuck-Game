...
<body>
    ...
    <form action="groups.php" method="post" id="form_groups">
        <input type="hidden" name="group_id" id="group_id">
        <div class="group-container">
            <div class="group-title">Groups</div>
            <a href="#" class="group" onclick="group1()">Group 1</a>
            <a href="#" type="button" class="group" onclick="group2()">Group 2</a>
            <a href="#" type="button" class="group" onclick="group3()">Group 3</a>
            <a href="#" type="button" class="group" onclick="group4()">Group 4</a>
            <a href="#" type="button" class="group" onclick="group5()">Group 5</a>
    </form>
    <script>
        function group1() {
            var data = "1"; // The data you want to pass
            document.getElementById("group_id").value = data; // Assign the data to the input box
            document.getElementById("form_groups").submit();
        }

        function group2() {
            var data = "2"; // The data you want to pass
            document.getElementById("group_id").value = data; // Assign the data to the input box
            document.getElementById("form_groups").submit();
        }

        function group3() {
            var data = "3"; // The data you want to pass
            document.getElementById("group_id").value = data; // Assign the data to the input box
            document.getElementById("form_groups").submit();
        }

        function group4() {
            var data = "4"; // The data you want to pass
            document.getElementById("group_id").value = data; // Assign the data to the input box
            document.getElementById("form_groups").submit();
        }

        function group5() {
            var data = "5"; // The data you want to pass
            document.getElementById("group_id").value = data; // Assign the data to the input box
            document.getElementById("form_groups").submit();
        }
    </script>
    ...
    <div class="actions-container">
        <button class="action-button" onclick="window.location.href = 'addplayers.php';">Add</button>
        <button class="action-button" onclick="window.location.href = 'editplayers.php';">Edit</button>
    </div>
    ...
    <div class="container">
        <div class="scrollable-container">
            ...
            $sql = "SELECT * FROM students WHERE teacherId = '$teacherId' AND classId = '$classId'";
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
                $count = 1;
                foreach ($result as $row) {
                    echo '<div class="textbox">&nbsp
                        <span><b>' . $count++ . '.</b> ' . $row['name'] . '</span>
                      </div>';
                }
                exit;
            } else {
